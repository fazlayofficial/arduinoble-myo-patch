/*
  ...
  PATCHED — Fix for ArduinoBLE issue #71
  writeValue() now uses ATT.writeCmd() (write-without-response) instead of
  ATT.writeReq() (blocking). Required for high-throughput notify devices
  like the Myo Armband that flood the ATT channel at 250+ notifications/sec,
  which prevents the write response from ever arriving.
*/
